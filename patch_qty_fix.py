...
old_block = """<td class="text-center fw-bold">
                                {% if entry.qty_returned > 0 and entry.qty_returned < entry.qty %}
                                    {{ entry.qty - entry.qty_returned }} <small class="text-muted fw-normal">({{ entry.qty_returned }} returned)</small>
                                {% else %}
                                    {{ entry.qty }}
                                {% endif %}
                            </td>"""

new_block = """<td class="text-center fw-bold align-middle">
                                {% if entry.qty_returned > 0 and entry.qty_returned < entry.qty %}
